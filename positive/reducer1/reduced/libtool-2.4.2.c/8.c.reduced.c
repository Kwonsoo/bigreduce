...
void *foreach_dirinpath_data2, *try_dlopen_tmp___29, *lt__malloc_mem;
char *find_file_callback_pdir;
int find_handle_callback;
lt_dlhandle try_dlopen_newhandle, lt_dlopenadvise_handle;
void *lt__zalloc();
...
try_dlopen(lt_dlhandle p1) {
  unload_deplibs(*p1);
  try_dlopen_tmp___29 = lt__zalloc();
  *p1 = try_dlopen_tmp___29;
  find_handle(&try_dlopen_newhandle);
  p1 = try_dlopen_newhandle;
}

lt_dlopenadvise() { try_dlopen(&lt_dlopenadvise_handle); }
