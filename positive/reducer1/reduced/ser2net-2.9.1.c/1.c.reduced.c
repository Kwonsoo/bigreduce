...
int handle_accept_port_read_cntlr_0, sel_select_once_i;
void *handle_accept_port_read_tmp, *sel_alloc_selector_tmp;
void handle_tcp_fd_read();
handle_tcp_fd_write() {
  airac_observe(controllers, 0);
...
sel_select_once(struct selector_s *p1) {
  p1->fds[sel_select_once_i].handle_read(sel_select_once_i,
                                         p1->fds[sel_select_once_i].data);
}
